...
export function TeamScorecard({ scores }: TeamScorecardProps) {
  ...
  function renderRow(holes: typeof COURSE_HOLES, label: string) {
    return (
      <div className="mb-3">
        <p className="text-[10px] tracking-[0.2em] uppercase text-text-muted mb-1 font-[family-name:var(--font-heading)]">
          {label}
        </p>
          ...
          {holes.map((h) => {
            ...
            return (
              <div key={h.hole_number} className="text-center">
                <div className="text-[9px] text-text-muted font-[family-name:var(--font-mono)]">
                  {h.hole_number}
                </div>
                <div className={`text-xs font-[family-name:var(--font-mono)] font-bold ${
                  diff === null ? "text-text-muted" :
                  diff < 0 ? "text-grateful-red" :
                  diff === 0 ? "text-foreground" : "text-text-secondary"
                }`}>
                  {strokes ?? "-"}
                </div>
              </div>
