    ...
    <div className="max-w-md mx-auto">
      ...
      <div className="flex justify-between items-center mb-6">
        <button
          ...
          className="text-text-muted text-sm hover:text-foreground transition-colors font-[family-name:var(--font-body)] cursor-pointer"
        >
          &larr; Change Team
        </button>
        <button
          onClick={() => setShowSummary(true)}
          className="text-accent text-sm hover:text-accent/80 transition-colors font-[family-name:var(--font-body)] cursor-pointer"
        >
          View Summary
        </button>
      </div>
      ...
      <div className="text-center mb-2">
        ...
        <span className={`ml-2 font-[family-name:var(--font-mono)] font-bold ${
          runningScoreToPar < 0 ? "text-grateful-red" : runningScoreToPar === 0 ? "text-foreground" : "text-text-muted"
        }`}>
          {formatScoreToPar(runningScoreToPar)}
        </span>
        <span className="ml-2 text-xs text-text-muted font-[family-name:var(--font-body)]">
      ...
      >
        <p className="text-xs tracking-[0.3em] uppercase text-text-muted mb-1 font-[family-name:var(--font-heading)]">
          Hole
        </p>
        ...
        <div className="flex justify-center gap-6 mb-8 text-sm">
          <span className="text-text-secondary font-[family-name:var(--font-body)]">
        ...
        <div className="flex items-center justify-center gap-6 mb-6">
          <button
            ...
            className="scoring-button w-14 h-14 rounded-full bg-gray-100 text-foreground text-2xl font-bold hover:bg-gray-200 transition-colors disabled:opacity-30 disabled:cursor-not-allowed cursor-pointer flex items-center justify-center"
          >
            -
          </button>
          <div className="relative">
            ...
            {savingHoles.has(currentHole) && (
              <span className="absolute -top-1 -right-3 w-2 h-2 bg-accent rounded-full animate-pulse" />
            )}
          </div>
          <button
            onClick={() => handleStrokeChange(displayStrokes + 1)}
      ...
      <div className="flex justify-between mt-6 gap-4">
        <button
          ...
          className="flex-1 py-3 text-sm font-[family-name:var(--font-heading)] uppercase tracking-wider text-text-secondary hover:text-foreground transition-colors disabled:opacity-30 disabled:cursor-not-allowed cursor-pointer"
        >
          &larr; Prev Hole
        </button>
...
function ScoringRoundSummary({ scores, onEditHole, onBack, onChangeTeam }: ScoringRoundSummaryProps) {
  ...
  function renderNine(holes: typeof COURSE_HOLES, label: string) {
    ...
    return (
      <div className="mb-6">
        <h3 className="text-xs tracking-[0.2em] uppercase text-text-muted mb-3 font-[family-name:var(--font-heading)]">
          {label}
        </h3>
        <div className="space-y-1">
            ...
            return (
              ...
              >
                <div className="flex items-center gap-4">
                  ...
                  <span className="text-text-secondary text-sm font-[family-name:var(--font-body)]">
                    Par {h.par}
                  </span>
                </div>
                <div className="flex items-center gap-4">
                  {strokes !== null ? (
                    <>
                      ...
                      <span className={`text-sm font-[family-name:var(--font-mono)] w-8 text-right ${
                        diff! < 0 ? "text-grateful-red font-bold" : diff === 0 ? "text-text-secondary" : "text-text-muted"
                      }`}>
          ...
          <div className="flex items-center justify-between py-2 px-3 border-t border-border mt-2">
            <span className="text-text-secondary text-sm font-[family-name:var(--font-heading)] uppercase tracking-wider">
              {label} Total
            </span>
            <div className="flex items-center gap-4">
              <span className="text-foreground font-[family-name:var(--font-mono)] font-bold">
              ...
              <span className="text-text-secondary text-sm font-[family-name:var(--font-mono)] w-8 text-right">
      ...
      <div className="glass-card p-6">
        ...
        <div className="text-center mb-6 pb-4 border-b border-border">
          <span className={`text-4xl font-bold font-[family-name:var(--font-mono)] ${
            totalToPar < 0 ? "text-grateful-red" : totalToPar === 0 ? "text-foreground" : "text-text-muted"
          }`}>
